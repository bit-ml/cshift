...
def create_panoptic_dict():
    ...
    panoptic_dict['info'] = {
        'description': 'Replica Generated Dataset',
...
def convert_categories(panoptic_dict, scene_dict):
    ...
    stuff_categories = ['wall', 'ceiling', 'floor']
    ...
    for scene_category in scene_dict['classes']:
        panoptic_dict['categories'].append({
            ...
            scene_category['name'],
            'isthing':
            int(scene_category['name'] not in stuff_categories),
            'id':
            scene_category['id'],
            'name':
            scene_category['name']
...
class Generator:
    ...
    def __init__(self, path):
        ...
        self._scenes = [
            ...
            "frl_apartment_4", "frl_apartment_5", "hotel_0", "office_0",
            "office_1", "office_2", "office_3", "office_4", "room_0", "room_1",
            "room_2"
        ]
        ...
        self._scene_to_rooms = {
            ...
            "office_0": [create_room(-0.98, 0.36, 1.21, 0.85, -1.56, 0.27)],
            "office_1": [create_room(-0.12, -0.50, 1.37, 0.64, 0.93, 0.30)],
            "office_2": [create_room(-0.25, -0.76, 1.05, 0.11, 3.26, 0.44)],
            "office_3": [create_room(-1.32, -3.80, 1.13, -0.29, 1.14, 0.22)],
            "office_4": [create_room(0.24, 2.51, 1.40, 3.45, -0.71, 0.40)],
            "room_0": [create_room(0.0, 0.00, 0.51, 4.9, 2.03, 0.02)],
            "room_1": [create_room(0.26, 0.22, 0.98, -3.82, -0.19, -0.13)],
            "room_2": [create_room(0.75, -1.13, 0.02, 4.22, -0.33, -1.15)]
        }
    ...
    def load_scene_semantic_dict(self, scene):
        with open(
                os.path.join(self._dataset_path, scene, 'habitat',
                             'info_semantic.json'), 'r') as f:
            return json.load(f)
    ...
    def fix_semantic_observation(self, semantic_observation, scene_dict):
        ...
        for id in np.unique(semantic_observation):
            if scene_dict['id_to_label'][id] < 0:
                ...
                print(
                    'Warning: unexpected id 0 occured, considered as unlabeled...'
                )
                print(scene_dict)
    ...
    def save_color_observation(self, observation, frame_number, out_folder):
        ...
        # Save to png
        # print("save_path_npy", save_path_npy)
        # color_img.save(
        #     os.path.join(out_folder,
        #                  self.filename_from_frame_number(frame_number)))
        ...
        np.save(save_path_npy, color_observation)
    ...
    def save_semantic_observation(self, observation, frame_number, out_folder,
        ...
        if not os.path.exists(out_folder):
            os.makedirs(out_folder)
        semantic_observation = self.fix_semantic_observation(
            observation["semantic_sensor"], scene_dict)
        semantic_img = Image.new(
            "I",
            (semantic_observation.shape[1], semantic_observation.shape[0]))
    ...
    def save_depth_observation(self, observation, frame_number, out_folder):
        ...
        save_path = os.path.join(
            out_folder,
            self.filename_from_frame_number(frame_number)).replace(".png", "")
        # depth_observation /= 10.
        np.save(save_path, depth_observation)
    ...
    def save_observations(self, observation, frame_number, out_folder,
        ...
        #     scene_dict)
        self.save_depth_observation(
            observation, frame_number,
            os.path.join(out_folder, split_name, 'depth'))

    def update_dict(self, panoptic_dict, scene_dict, frame_number, out_folder,
                    split_name, scene, state):
        ...
        for id in np.unique(self._last_semantic_frame):
            ...
            if label > 0:  # == 0 would be undefined -> no annotation
                panoptic_dict['annotations'][-1]['segments_info'].append({
                    'id':
                    int(id),  # the number in the semantic image
                    'category_id':
                    int(label),  # the matching category id 
                    'iscrowd':
                    0,
                    'bbox':
                    [int(minx),
                     ...
                     int(maxx - minx),
                     int(maxy - miny)
                     ],  # x, y, (starting top left) width, height
                    'area':
                    int(np.sum(
                        mask))  # area in pixels (exact, not bounding box)
                })

    def save_dict(self, panoptic_dict, out_folder, split_name):
        with open(
                os.path.join(out_folder, 'annotations',
                             f"panoptic_{split_name}.json"), 'w') as f:
    ...
    def generate(self, out_folder, split_name, frames_per_room=100):
        ...
        for scene in self._scenes:
            # setup the simulator
            settings["scene"] = os.path.join(self._dataset_path, scene,
            ...
            for room in self._scene_to_rooms[scene]:
                for _ in range(0, frames_per_room):
                    ...
                    random_state.position[0] = np.random.uniform(
                        room['x_min'], room['x_max'])
                    random_state.position[1] = np.random.uniform(
                        room['z_min'], room['z_max'])
                    random_state.position[2] = -np.random.uniform(
                        room['y_min'], room['y_max'])
                    random_state.rotation = (
                        quat_from_angle_axis(np.random.uniform(0, np.pi),
                                             np.array([0, 1, 0])) *
                        quat_from_angle_axis(
                            np.random.uniform(-np.pi / 3, np.pi / 16),
                            np.array([1, 0, 0])) * quat_from_angle_axis(
                    ...
                    self.save_observations(observations, current_frame,
                                           out_folder, split_name,
                                           scene_semantic_dict)

                    # self.update_dict(panoptic_dict, scene_semantic_dict,
                    #                  current_frame, out_folder, split_name,
                    #                  scene, random_state)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Folder containing Replica dataset metadata, use the download script from https://github.com/facebookresearch/Replica-Dataset",
        default="/data/multi-domain-graph/datasets/replica_metadata")
    parser.add_argument(
        "--output",
        type=str,
        help="Output folder",
        default="/data/multi-domain-graph/datasets/replica_raw")
