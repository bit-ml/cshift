...
#        block_5_pre_dense_256 = self.pre_dense_6(block_5_add) # if error uncomment
    ...
    # def save_pred_to_disk(self, edge_maps, output_path_img):
    #     assert(len(edge_maps.shape) == 3)
    #     conc_img = np.concatenate(edge_maps.tolist())
    #     cv2.imwrite(output_path_img, conc_img)
    ...
    #     image = self.image_normalization(self.input_image)

    #     if torch.cuda.is_available():
    #         image = image.cuda()

    #     return image
    #     x = self.preproc_image(x)

    # def save_pred_to_disk(self, em_maps, output_path_img):
    #     import os
    ...
    #     em_a = em_maps.mean(axis=0)
    #     em_maps = torch.cat([em_maps, em_a[None]])

    #     em = em_maps[len(em_maps)-2]
    #     em[em < self.testing_threshold] = 0.0
    #     em_a[em_a < self.testing_threshold] = 0.0
    #     em = 255.0 * (1.0 - em)
    #     em_a = 255.0 * (1.0 - em_a)
    #     em = np.tile(em[:, :, None], [1, 1, 3])
    #     em_a = np.tile(em_a[:, :, None], [1, 1, 3])

    #     em = Image.fromarray(np.uint8(em))
    #     em_a = Image.fromarray(np.uint8(em_a))

    #     conc_img = np.concatenate([em, em_a])
    #     cv2.imwrite(output_path_img, conc_img)
    #     print("Saved to", output_path_img)
