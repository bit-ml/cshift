...
from .third_party.cython.connectivity import enforce_connectivity
...
def shift9pos(input, h_shift_unit=1, w_shift_unit=1):
    ...
    center = input_pd[:, h_shift_unit:-h_shift_unit,
                      w_shift_unit:-w_shift_unit]
...
def update_spixl_map(spixl_map_idx_in, assig_map_in):
    ...
    if (id_h == h) and (id_w == w):
        ...
        spixl_map_idx = F.interpolate(spixl_map_idx_in,
                                      size=(h, w),
                                      mode='nearest')

    assig_max, _ = torch.max(assig_map, dim=1, keepdim=True)
    assignment_ = torch.where(assig_map == assig_max,
                              torch.ones(assig_map.shape).cuda(),
                              torch.zeros(assig_map.shape).cuda())
    new_spixl_map_ = spixl_map_idx * assignment_  # winner take all
    new_spixl_map = torch.sum(new_spixl_map_, dim=1,
                              keepdim=True).type(torch.int)
...
def postproc_labels(spix_index, n_spixels=600, b_enforce_connect=False):
    spix_index_np = spix_index.detach().cpu().numpy()
    bs, h, w = spix_index_np.shape

    if b_enforce_connect:
        spix_index_np = spix_index_np.astype(np.int64)
        segment_size = (h * w) / (int(n_spixels) * 1.0)
        min_size = int(0.06 * segment_size)
        max_size = int(3 * segment_size)
        spix_index_np = enforce_connectivity(spix_index_np, min_size, max_size)
    return spix_index_np
